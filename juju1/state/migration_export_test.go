...
import (
	...
	"github.com/juju/1.25-upgrade/juju1/state/cloudimagemetadata"
...
func (s *MigrationBaseSuite) setLatestTools(c *gc.C, latestTools version.Number) {
	dbModel, err := s.State.Environment()
...
func (s *MigrationBaseSuite) primeStatusHistory(c *gc.C, entity statusSetter, statusVal Status, count int) {
...
func (s *MigrationExportSuite) checkStatusHistory(c *gc.C, history []description.Status, statusVal Status) {
...
func (s *MigrationExportSuite) TestModelUsers(c *gc.C) {
	...
	bob, err := s.State.AddEnvironmentUser(bobTag, s.Owner, "")
	c.Assert(err, jc.ErrorIsNil)
...
func (s *MigrationExportSuite) assertMachinesMigrated(c *gc.C, cons constraints.Value) {
	...
	c.Assert(err, jc.ErrorIsNil)
	s.primeStatusHistory(c, machine1, StatusStarted, addedHistoryCount)
	...
	s.checkStatusHistory(c, history[:addedHistoryCount], StatusStarted)
...
func (s *MigrationExportSuite) assertMigrateApplications(c *gc.C, cons constraints.Value) {
	...
	c.Assert(err, jc.ErrorIsNil)
	s.primeStatusHistory(c, application, StatusActive, addedHistoryCount)
	...
	history := exported.StatusHistory()
	c.Assert(history, gc.HasLen, expectedHistoryCount)
	s.checkStatusHistory(c, history[:addedHistoryCount], StatusActive)
}
...
func (s *MigrationExportSuite) TestUnits(c *gc.C) {
	...
	c.Assert(err, jc.ErrorIsNil)
	s.primeStatusHistory(c, unit, StatusActive, addedHistoryCount)
	s.primeStatusHistory(c, unit.Agent(), StatusIdle, addedHistoryCount)
	...
	s.checkStatusHistory(c, workloadHistory[:addedHistoryCount], StatusActive)

	agentHistory := exported.AgentStatusHistory()
	c.Assert(agentHistory, gc.HasLen, expectedHistoryCount)
	s.checkStatusHistory(c, agentHistory[:addedHistoryCount], StatusIdle)
