...
GOPATH=$WORK go get -v -d github.com/juju/1.25-upgrade/juju1/...

echo "Setting juju-core tree to $TAG."
(cd "${WORK}/src/github.com/juju/1.25-upgrade/juju1/" && bzr revert -r $TAG)

echo "Updating juju-core dependencies to the required versions."
GOPATH=$WORK godeps -u "${WORK}/src/github.com/juju/1.25-upgrade/juju1/dependencies.tsv"

# Smoke test
GOPATH=$WORK go build -v github.com/juju/1.25-upgrade/juju1/...

# Change the generic release to the proper juju-core version.
VERSION=$(sed -n 's/^const version = "\(.*\)"/\1/p' \
    $WORK/src/github.com/juju/1.25-upgrade/juju1/version/version.go)
